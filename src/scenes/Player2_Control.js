export default function Player2_Control(scene,input){
    if(scene.state.current === scene.player2){
        if(scene.player2.state.isActive){
                ...
                if(scene.player2.state.isRun){
                    if(scene.player2.x < 480){
                        if(scene.player2.state.haveBall){
                            scene.state.current===scene.player1? scene.ball.x = scene.player2.x +30 : scene.ball.x = scene.player2.x+24;
            ...
            if (input.x && input.z){
            ...
            else if (scene.player2.state.haveBall && Phaser.Input.Keyboard.JustDown(scene.keys.z) && scene.player2.state.canThrow){
                ...
                if(scene.player2.state.haveBall && scene.player2.state.canThrow && Phaser.Input.Keyboard.JustDown(scene.keys.z)){
                    ...
                    scene.catchBall()
                }else if(!scene.player2.state.haveBall && scene.state.turn === 'us' && scene.player2.state.onFloor && Phaser.Input.Keyboard.JustDown(scene.keys.z)){
                    scene.player2.anims.play(`${scene.player2.name}-pick`);
                    if(scene.player2.body.touching.up){
                        scene.pickBall()
                    }
                }
            }
                ...
                if(scene.player2.state.haveBall && scene.player2.state.canThrow && Phaser.Input.Keyboard.JustDown(scene.keys.x)){
                    scene.pass();
                }else if(!scene.player2.state.haveBall && scene.state.turn === 'us' && scene.player2.state.onFloor && Phaser.Input.Keyboard.JustDown(scene.keys.x)){
                    scene.player2.anims.play(`${scene.player2.name}-pick`);
                    if(scene.player2.body.touching.up){
                        scene.pickBall()
                    }
